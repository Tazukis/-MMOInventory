...
public class GuiListener implements Listener {
    ...
    public void registerClicks(InventoryClickEvent event) {
        if (event.getInventory().getHolder() instanceof PlayerInventoryView) {
            if (event.isCancelled()) ((PlayerInventoryView) event.getInventory().getHolder()).whenCanceled(event);
            else ((PlayerInventoryView) event.getInventory().getHolder()).whenClicked(event);
        }
    }

    @EventHandler(priority = EventPriority.HIGH, ignoreCancelled = true)
    public void registerDrags(InventoryDragEvent event) {
        if (event.getInventory().getHolder() instanceof PlayerInventoryView) event.setCancelled(true);
    }
